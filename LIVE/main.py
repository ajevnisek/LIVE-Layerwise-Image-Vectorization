...
def parse_args():
    ...
    parser.add_argument("--new-shape", type=int, default=240,
                        help='emojis are resized to 240x240, we would like to resize the images to this size. \n'
                             'If this is -1 then the original shape is used.')
...
if __name__ == "__main__":
    ...
    cfg.experiment_dir = \
        osp.join(cfg.log_dir, '{}_{}'.format(cfg.seed, '_'.join(cfg.signature)))
    if not os.path.exists(cfg.experiment_dir):

        timing_dir = os.path.join(cfg.experiment_dir, 'timing')
        ...
        gt_pil_image = PIL.Image.open(cfg.target)
        if cfg.new_shape > 0:
            gt_pil_image = gt_pil_image.resize((cfg.new_shape, cfg.new_shape))
        gt = np.array(gt_pil_image)
        ...
        for path_idx, pathn in enumerate(path_schedule):
            data_to_add = {f"{path_idx + 1}_start": time.time()}
            add_to_file(data_to_add, timing_file)
            ...
            if cfg.save.video:
                ...
                data_to_add = {f"{path_idx + 1}_end": time.time()}
                add_to_file(data_to_add, timing_file)

        print("The last loss is: {}".format(loss.item()))
        data_to_add = {f"{len(path_schedule) + 1}_total_end": time.time()}
        add_to_file(data_to_add, timing_file)
